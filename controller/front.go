...
import (
	...
	"github.com/lbb4511/sand/setting"
	"github.com/lbb4511/sand/utils"
)
...
func init() {
	...
	// 匹配模版
	Tmpl = template.New("eiblog").Funcs(utils.TplFuncMap)
	files := utils.ReadDir("views", func(name string) bool {
		if name == ".DS_Store" {
			return true
		}
		return false
...
func UserCookie(c *gin.Context) {
	...
	if err != nil || cookie == "" {
		c.SetCookie("u", utils.NewV4().String(), 86400*730, "/", "", true, true)
	}
}
	...
	if err != nil {
		...
		for i, v := range postsList.Response {
			...
			dcs.Data.Comments[i] = commentsDetail{
				...
				CreatedAtStr: utils.ConvertStr(v.CreatedAt),
				Message:      v.Message,
...
func HandleDisqusCreate(c *gin.Context) {
	...
	resp.Data = commentsDetail{
		...
		CreatedAtStr: utils.ConvertStr(postDetail.Response.CreatedAt),
